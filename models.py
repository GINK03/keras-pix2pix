...
def generator_unet_upsampling(img_dim, bn_mode, model_name="generator_unet_upsampling"):
  ...
  for i, f in enumerate(list_nb_filters[1:]):
    conv = conv_block_unet(list_encoder[-1], f, "unet_conv2D_%s"%(i + 2), bn_mode, bn_axis)
  ...
  # Decoder
  list_decoder = [up_conv_block_unet(list_encoder[-1], list_encoder[-2], list_nb_filters[0], "unet_upconv2D_1", bn_mode, bn_axis, dropout=True)]
  for i, f in enumerate(list_nb_filters[1:]):
    ...
    if i < 2:
      ...
      d = False
    conv = up_conv_block_unet(list_decoder[-1], list_encoder[-(i + 3)], f, "unet_upconv2D_%s"%(i + 2), bn_mode, bn_axis, dropout=d)
...
def DCGAN_discriminator(img_dim, nb_patch, bn_mode, model_name="DCGAN_discriminator", use_mbd=True):
  ...
  list_input = [Input(shape=img_dim, name="disc_input_{}".format(i) ) for i in range(int(nb_patch))]
